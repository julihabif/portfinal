...
const StickerPeel: React.FC<StickerPeelProps> = ({
  ...
  // Initialize GSAP Draggable with proper position handling
  useEffect(() => {
    const target = dragTargetRef.current;
    if (!target) return;

    const initDraggable = () => {
      // Get the section container as bounds (not just parentNode)
      const boundsEl = target.closest('section') || document.body;

      // Calculate initial position
      let initialX = 0;
      let initialY = 0;

      if (typeof initialPosition === 'object' && initialPosition.x !== undefined) {
        ...
        const targetWidth = width + (defaultPadding * 2);
        if (parentRect) {
          initialX = (parentRect.width - targetWidth) / 2;
          initialY = (parentRect.height - targetWidth) / 2;
        }
      }

      // CRITICAL FIX: Set initial position using GSAP, not left/top
      gsap.set(target, {
        x: initialX,
        y: initialY,
    ...
    if (img && !img.complete) {
      img.addEventListener('load', initDraggable, { once: true });
    } else {
      // Image already loaded, initialize on next frame
  ...
  }, [initialPosition, width, defaultPadding]);
